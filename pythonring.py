...
def generate_rings(
    ...
    vae = ConvVAE(batch_size= 1000, is_training=False)
       # Load model
    ...
    def rings_to_lidar(rings, N_RAYS=360):
        CHANNEL = 0
        i_to_j, j_to_ii = generate_downsampling_map(N_RAYS, angle_levels)
    ...
    def visualize_rings(
        ...
        plt.savefig("/home/ning/lidarprojectR/photo/Visual.png")                  
        return ax
    def visualize_r(ring,B):
        if B==0:
           ...
           plt.axis('off')
           plt.savefig("/home/ning/lidarprojectR/photo/ring_rep.png")
        else:
           ...
           plt.imshow(ring, cmap=cmap, interpolation='none', origin='lower')
           plt.axis('off')
           plt.savefig("/home/ning/lidarprojectR/photo/en_decode_ring_rep.png")
    def encoder(rings):
        plt.ion()
        ...
        #visualize_lidar_data(rings_to_lidar(rings_compress).reshape(-1))
        ...
        print(ring_accuracy)
        #visualize_rings(rings,scans)
        ring_def["visualize_r"](rings_compress,1)
        ring_def["visualize_r"](rings,0 )
        return rings_compress,rings_encode
    def encoder2(scans,B=0):
        if B!=0:
          #visualize_lidar_data(scans)
          rings = lidar_to_rings(scans)
          visualize_rings(rings[0, :, :, :], scan=scans[0])
          encoder(rings)
          rings_encode = vae.encode(rings)
        else:
          rings = lidar_to_rings(scans)
...
def visualize_lidar_data(ranges, max_range=10.2):
    plt.figure("Lidar")
    plt.clf()
    angles = np.linspace(np.pi, np.pi- 2* np.pi, len(ranges))
    ...
    plt.savefig(f"/home/ning/lidarprojectR/photo/Lidar_{random_number}.png")
